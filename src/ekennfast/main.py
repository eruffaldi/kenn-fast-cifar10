...
from .layers import KernelConv2d, GaussianKernel, PolynomialKernel
...
def test(model, criterion, device, data_loader,what):
    model.eval()
    test_loss = 0
    correct = 0
    y_pred = []
    ...
    with torch.no_grad():
        for data, otarget in data_loader:
    ...
    accuracyvalue = 100. * correct / len(data_loader.dataset)
    print('\n{} set: Accuracy: {}/{} ({:.2f}%)\n'.format(what,
        correct, len(data_loader.dataset),accuracyvalue))
...
def ctrlc_handler(signal_received, frame):
    ...
    global stop_requested
    stop_requested = True
    print("wait closing... train is interrupted at END of epoch, test is performed, checkpoint is created")
def main():
    targetaccuracy = 94
    ...
    parser.add_argument('--benchout',action="store_true")    
    parser.add_argument('--dataset', choices=["cifar10","mnist"], default="mnist")
    parser.add_argument('--model', choices=["cnn","softmax","kenn"], default="cnn")
    ...
    elif args.model == "kenn":
        model = NetKENN(insize,inchannels,**kwmodel).to(device)
    else:
    ...
    maxaccuracyvalue = 0
    t = time.time()
    benchoutjson = dict(version="v1.0",author="Emanuele Ruffaldi",authorEmail="emanuele.ruffaldi@gmail.com",
            codeURL= "https://github.com/apple/ml-cifar-10-faster",
            framework= "Pytorch 1.0.2",
            model= "Custom KENN",
            hardware= "4-CPU",
            top1Accuracy=0,
            latency=0,
    ...
    benchout = csv.DictWriter(open(go + ".bench.tsv","w"),delimiter="\t",fieldnames=["epoch","hours","top1Accuracy","trainAccuracy"])
    benchout.writeheader()
    for epoch in range(1, args.epochs + 1):
        if stop_requested:
            break
        train(args, model, criterion, device, train_loader, optimizer, epoch)
        print("Testing Training Dataset")
        traccuracyvalue,trcf_matrix = test(model,criterion, device, train_loader,"train")
        print("Testing Test Dataset")
        tt = time.time()
        accuracyvalue,cf_matrix = test(model,criterion, device, test_loader,"test")
        test_time = time.time()-tt
        scheduler.step()
        benchoutjson["latency"] = test_time*1000/len(dataset2)
        benchout.writerow(dict(epoch=epoch,top1Accuracy=accuracyvalue,hours=(tt-t)/3600.0,trainAccuracy=traccuracyvalue))
        maxaccuracyvalue = max(accuracyvalue,maxaccuracyvalue)
    training_time = time.time()-t
    benchout["top1Accuracy"] = maxaccuracyvalue # in 100%
    ...
    json.dump(benchoutjson,open(go+".bench.json","w"))
